...
use crate::runtime::tray_window::Program;
use crate::runtime::user_interface::{self, UserInterface};
use crate::runtime::Debug;
use crate::settings::LiveTraySettings;
use crate::settings::TraySettings;
use crate::{Clipboard, Error, Proxy, Settings};
use window_manager::WindowManager;
use winit::tray::Tray;
use winit::tray::TrayBuilder;
    ...
    Self::Theme: DefaultStyle,
{
    ...
    /// Returns the current [`TraySettings`] of the `window` of the [`Application`].
    fn tray_settings(&self) -> LiveTraySettings<Self::Message>;
    ...
    A::Theme: DefaultStyle,
{
    ...
    let (tray, tray_window) = {
        let tray = TrayBuilder::new().with_tooltip(&tray_settings.title);
        let tray = if let Some(icon) = tray_settings.icon {
            if let Some(icon) = conversion::icon(icon) {
                tray.with_icon(icon)
            } else {
                tray
            }
        ...
        let child_window = winit::window::WindowBuilder::new()
            .with_inner_size(winit::dpi::PhysicalSize::new(
                tray_settings.width,
                tray_settings.height,
            ))
        ...
        (
            Arc::new(
                tray.build(&event_loop)
                    .map_err(Error::WindowCreationFailed)?,
            ),
            Arc::new(child_window),
        )
    };
    ...
    A::Theme: DefaultStyle,
{
    ...
    // add tray window to window manager and user interfaces and caches
    let tray_window = window_manager
        .get_mut(window::Id::TRAY)
        .expect("Get tray window");

    let tray_thingy = build_user_interface(
        &application,
        user_interface::Cache::default(),
        &mut tray_window.renderer,
        tray_window.state.logical_size(),
        &mut debug,
        window::Id::TRAY,
    );

    let _ = user_interfaces.insert(window::Id::TRAY, tray_thingy);

    let _ =
        ui_caches.insert(window::Id::TRAY, user_interface::Cache::default());
            ...
            Event::EventLoopAwakened(event) => {
                match event {
                    ...
                    event::Event::WindowEvent {
                            ...
                            if window_id == tray.id() {
                                match window_event {
                                    winit::event::WindowEvent::CursorMoved {
                                        ...
                                        let tray_window = window_manager.get_mut(
                                            window::Id::TRAY,
                                        ).unwrap();
                                        
                                        let size: winit::dpi::PhysicalSize<
                                            f64,
                                        > = tray_window.raw.outer_size().cast();
                                        ...
                                        tray_window.raw.set_outer_position(pos)
                                    }
                                    winit::event::WindowEvent::MouseInput {
                                        ...
                                        /// handles the tray input.
                                        macro_rules! handle_tray_input {
                                            ($window_event:expr) => {
                                                match $window_event {
                                                    crate::settings::LiveTrayOptions::CloseAll => break 'main,
                                                    crate::settings::LiveTrayOptions::OpenWindow(id) => {
                                                        let maybe_window = window_manager.get_mut(
                                                            id,
                                                        );
                                                        if let Some(window) = maybe_window  {
                                                            window.raw.set_visible(true);
                                                            window.raw.set_minimized(false);
                                                            window.raw.focus_window();
                                                        }
                                                    }
                                                    crate::settings::LiveTrayOptions::Message(m) => proxy.send(m),
                                                    crate::settings::LiveTrayOptions::OpenTrayWindow => {
                                                        let tray_window = window_manager.get_mut(
                                                            window::Id::TRAY,
                                                        ).unwrap();
                                                        tray_window.raw.set_visible(true);
                                                        tray_window.raw.focus_window();
                                                    }
                                                    crate::settings::LiveTrayOptions::None => (),
                                                }
                                            };
                                        }
                                        ...
                                        {
                                            handle_tray_input!(application.tray_settings().right_click)
                                        }
                                        if button == winit::event::MouseButton::Middle
                                            && state
                                                == winit::event::ElementState::Released
                                        {
                                            handle_tray_input!(application.tray_settings().middle_click)
                                        }
                                        if button == winit::event::MouseButton::Left
                                            && state
                                                == winit::event::ElementState::Released
                                        {
                                            handle_tray_input!(application.tray_settings().left_click)
                                        }
                                    }
                                    _ => (),
                                }
                        ...
                        if id == window::Id::TRAY && window_event == winit::event::WindowEvent::Focused(false) {
                            let tray_window = window_manager.get_mut(
                                window::Id::TRAY,
                            ).unwrap();
                            tray_window.raw.set_visible(false);
                            continue;
                        }
                        ...
                        ) && window.exit_on_close_request
                        {
                            ...
                            if window_manager.iter_mut().all(|(id, _)| id == window::Id::TRAY) {
                                break 'main;
                            }
                        } else {
                            window.state.update(
