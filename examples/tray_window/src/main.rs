use iced::event;
use iced::executor;
use iced::settings::{TraySettings, LiveTrayOptions, LiveTraySettings};
use iced::tray_application::{self, Application};
...
fn main() -> Result<(), iced::Error> {
    ...
    Example::run(Settings::default(), tray_settings)
}
...
impl tray_application::Application for Example {
        ...
        (
            Example {
                windows: HashMap::from([
                    (window::Id::MAIN, Window::new(1)),
                    (window::Id::TRAY, Window::new(1)),
                ]),
        ...
        match message {
            ...
            Message::WindowOpened(id, position) => {
                if let Some(position) = position {
                    self.next_window_pos =
                        window::Position::Specific(position + Vector::new(20.0, 20.0));
                }
    ...
    fn view(&self, window: window::Id) -> Element<Message> {
        ...
        if window == window::Id::TRAY {
            return self.tray_view();
        }
    ...
    fn tray_settings(&self) -> LiveTraySettings<Self::Message> {
        LiveTraySettings {
            left_click: LiveTrayOptions::OpenWindow(window::Id::MAIN),
            right_click: LiveTrayOptions::OpenTrayWindow,
            middle_click: LiveTrayOptions::CloseAll,
        }
    }
}

impl Example {
    fn tray_view (&self) -> Element<Message> { 
        

        container(iced::widget::column![
            iced::widget::text("Tray Window"),
            iced::widget::text("Right click to open the tray window"),
            iced::widget::text("Left click to open the main window"),
            iced::widget::text("Middle click to close all windows"),
        ].spacing(10))
            .width(Length::Fill)
            .height(Length::Fill).padding(1)
            .center_x()
            .center_y()
            .into()
    }
